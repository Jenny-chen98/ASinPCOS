...
## Plot1: statistic of DASEs distribution from rMATS
...
## Relationship between DEG and DASEG
...
onlydeg <- deg[deg$change!="NOT",]
onlydeg.list <- unique(onlydeg$external_gene_name)

dase.list <- unique(dase$geneSymbol)

library(ggvenn)

list <- list("DEG" = onlydeg.list,
             "DASEG" = dase.list)

intersect(onlydeg.list, dase.list)

pdf("../results/02_rmats_describe/venn_DEG_DASEG.pdf")
ggvenn(list, 
       show_elements = FALSE,
...
dn <- read.table("../results/pcos_rmats_out/rMAPS2/resultMaps/pVal.dn.vs.bg.RNAmap.txt",
                 sep = "\t", header = T)
up <- read.table("../results/pcos_rmats_out/rMAPS2/resultMaps/pVal.up.vs.bg.RNAmap.txt",
                 sep = "\t", header = T)

dn$protein <- unlist(strsplit(dn$RBP, "[.]"))[seq(1,2*nrow(dn),2)]
dn$motif <- unlist(strsplit(dn$RBP, "[.]"))[seq(2,2*nrow(dn),2)]
dn$group <- "dn"

up$protein <- unlist(strsplit(up$RBP, "[.]"))[seq(1,2*nrow(up),2)]
up$motif <- unlist(strsplit(up$RBP, "[.]"))[seq(2,2*nrow(up),2)]
up$group <- "up"

rmaps <- rbind(dn,up)

rmaps$symbol <- rmaps$protein

rmaps$symbol <- gsub("HuR","ELAVL1",rmaps$symbol)
rmaps$symbol <- gsub("SRp20","SRSF3",rmaps$symbol)
rmaps$symbol <- gsub("SRp40","SRSF5",rmaps$symbol)
rmaps$symbol <- gsub("Tra2-beta","TRA2B",rmaps$symbol)
rmaps$symbol <- gsub("SF2-ASF","SRSF1",rmaps$symbol)

rbp <- unique(rmaps$symbol)

load("../data/R/DEG.Rdata")
onlydeg <- filter(deg, change != "NOT")

table(rbp %in% onlydeg$external_gene_name)


# 
rmaps$count <- as.numeric(1)

rmaps2 <- aggregate(count ~ symbol, rmaps, sum) %>% 
  arrange(desc(count))
...
## Statistic PLOT of IsoformSwitchAnalyzeR results
...
# Compare DASEGs from rMATS and DASGs from isoformswitchanalyzeR
...
# GO enrichment of the intersection set

```{r}
interg <- intersect(dasg.isoform, dasg.rmats)
library(xlsx)
load("../data/R/fpkm.Rdata")

dase.fpkm <- fpkm[fpkm$external_gene_name %in% interg,] %>% na.omit()
dase.fpkm2 <- dase.fpkm[!duplicated(dase.fpkm$external_gene_name),]
...
write.xlsx(dase.kmeans$long.res, "../results/04_intersect/intersect_kmeans_dase.xlsx",
           row.names = F, col.names = T)
...
write.xlsx(dase.enrich2, file = "../results/04_intersect/intersect_clustergvis_dase.enrich.xlsx")
write.xlsx(dase.enrich, file = "../results/04_intersect/intersect_clustergvis_dase.enrichFilt.xlsx")
...
visCluster(object = dase.kmeans,
           ...
           # markGenes.side = "left",
           annoTerm.data = dase.enrich,
           go.col = rep(ggsci::pal_d3()(2),each = 5),
           line.side = "none",
           sample.group = c(rep("Normal",6),rep("PCOS",5)),
           genes.gp = c('italic',fontsize = 12,col = "black"),
           border = FALSE,
...
# dase.fpkm3 <- dase.fpkm2[apply(dase.fpkm2, 1, function(x) sum(x)>2),]
# dase.fpkm.g <- data.matrix(dase.fpkm3)
# dase.fpkm.g <- new('ExpressionSet', exprs=dase.fpkm.g)
# library(Mfuzz)
# dase.fpkm.g <- filter.NA(dase.fpkm.g, thres=0.25) # filter missing values
# dase.fpkm.g <- filter.std(dase.fpkm.g,min.std=0)
# 
# dase.s <- standardise(dase.fpkm.g)
# 
# dase.mfuzz <- clusterData(exp = dase.fpkm2,cluster.method = "mfuzz",
#                           cluster.num = 5)
```
